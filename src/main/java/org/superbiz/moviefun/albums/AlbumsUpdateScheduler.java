...
public class AlbumsUpdateScheduler {
    ...
    private boolean startAlbumSchedulerTask() {
        int rowsUpdated = jdbcTemplate.update("UPDATE album_scheduler_task SET started_at = CURRENT_TIMESTAMP() WHERE started_at <= timestamp(DATE_SUB(NOW(), INTERVAL 2 MINUTE)) OR started_at IS NULL");
